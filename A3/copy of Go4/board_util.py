...
class GoBoardUtil(object):
    ...
    def generate_all_policy_moves(board,pattern,check_selfatari):
        ...
        if board.last_move != None:
            atari_cap_moves = GoBoardUtil.atari_cap(board)
            if atari_cap_moves != None:
                
                atari_cap_moves = GoBoardUtil.filter_moves(board, atari_cap_moves, check_selfatari)
                if len(atari_cap_moves) > 0:
    ...
    def atari_cap (board):
        ...
        moves = []
        cap_move = board.last_liberty_pos(last)
        if cap_move:
            moves.append(cap_move)
        else:
            moves
        return moves

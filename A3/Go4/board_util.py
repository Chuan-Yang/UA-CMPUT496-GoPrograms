...
class GoBoardUtil(object):
    ...
    def generate_all_policy_moves(board,pattern,check_selfatari):
        ...
        if (board.last_move != None):
            # Attack
            if (board._liberty(board.last_move, GoBoardUtil.opponent(board.current_player))) == 1:
                atari_attack = board.last_point
                if (board.ko_constraint != atari_attack and not GoBoardUtil.selfatari_filter(board, atari_attack, board.current_player)):
                    return  [atari_attack], "AtariCapture"
        
            # Defense
            no_need = False
            atari_defense = []
                
            board.visited = np.zeros((board.maxpoint))
            for x in range(1,board.size+1):
                for y in range(1,board.size+1):
                    board.flood_neighbor = []
                    stone = board._coord_to_point(x,y)
                    if (board.board[stone] == board.current_player and board.visited[stone] == 0):
                        if (board._liberty(stone, board.current_player)) == 1:

                            neighbors = board.flood_neighbor
                            if (board.last_move not in neighbors):
                                no_need = True
                                break

                            #Run away
                            move_point = board.last_point
                            board._play_move(move_point, board.current_player)
                            #print ((board._liberty(stone, board.current_player)))
                            if (board._liberty(stone, board.current_player)) != 1:
                                if (move_point not in atari_defense):
                                    atari_defense.append(move_point)
                            board.board[move_point] = EMPTY
                        
                            # Capture
                            for opp_stone in neighbors:
                                if (board._liberty(opp_stone, GoBoardUtil.opponent(board.current_player))) == 1:
                                    if (board.last_point not in atari_defense and (board.ko_constraint != atari_defense)) and not GoBoardUtil.selfatari_filter(board, board.last_point, board.current_player):
                                        atari_defense.append(board.last_point)
                if (no_need):
                    break

            if (atari_defense != []):
                return  atari_defense, "AtariDefense"        
    ...
    def generate_move_with_filter(board, use_pattern, check_selfatari):
        ...
        move = None
        if use_pattern:
            moves, way = GoBoardUtil.generate_all_policy_moves(board,use_pattern, check_selfatari)

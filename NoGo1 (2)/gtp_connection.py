...
class GtpConnection():
    ...
    def get_cmd(self, command):
        ...
        if self.arg_error(command_name, len(args), args):
            return
    ...
    def arg_error(self, cmd, argnum, args):
        ...
        False otherwise
        """
        if cmd in self.argmap and (self.argmap[cmd][0] > argnum or self.argmap[cmd][0] < argnum):
                self.error("illegal move: {} wrong number of arguments".format(' '.join(args)))
                return True

        if cmd == 'play' and (args[0] != 'b' and args[0] != 'w'):
                self.error("illegal move: {} wrong color".format(' '.join(args)))
                return True

        #if cmd in self.argmap and (ord(args[1][0]) - ord('a') + 1 > self.board.size and ord(args[1][0]) - ord('a') + 1 <= 0) and (ord(args[1][1]) - 48 > self.board.size and ord(args[1][1]) - 48 <= 0):
        #        self.error("Illegal Move: {} wrong coordinate".format(' '.join(args)))
        #        return True
    ...
    def play_cmd(self, args):
        ...
        try:
            ...
            if args[1].lower()=='pass':
                ...
                self.error("illegal move: {} wrong cordinate".format(' '.join(args)))
                return
